...
class UI:
    ...
    def draw(self, screen, level):
        ...
        pollution_index = min(10, int(level.pollution / 10)) 
        screen.blit(self.bars[pollution_index], (90, 10))

        # Прочность
        screen.blit(self.durability_icon, (10, 100))
        durability_index = min(10, int(level.player.durability)) 
        screen.blit(self.bars[durability_index], (90, 100))

        # Вместимость
        screen.blit(self.capacity_icon, (10, 190))
        capacity_index = min(10, int(level.player.capacity / level.player.max_capacity * 10)) 
